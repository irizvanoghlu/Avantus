...
DIR = Path('./test/test_model_params/')
JSON = '.json'
CSV = '.csv'
...
def test_da_12hr():
    assert_ran(DIR / f'019a-DA_battery_month_12hropt{JSON}')


def test_da_month5():
    assert_ran(DIR / f'023-DA_month_results_dir_label{JSON}')
...
def test_da_month_degradation_battery_replaced_during_optimization():
    assert_ran(DIR / f"010-degradation_test{JSON}")
...
def test_da_deferral_month():  # should run for years: 2017 2023 2024
    assert_ran(DIR / f'003-DA_Deferral_battery_month{JSON}')
...
def test_da_user_month():
    assert_ran(DIR / f'011-DA_User_battery_month{JSON}')


def test_da_ra_month():
    assert_ran(DIR / f'012-DA_RApeakmonth_battery_month{JSON}')
...
def test_da_ra_month2():
    assert_ran(DIR / f'014-DA_RApeakyear_battery_month{JSON}')


def test_da_dr_month():
    assert_ran(DIR / f'015-DA_DRdayahead_battery_month{JSON}')


def test_da_dr_month1():
    assert_ran(DIR / f'016-DA_DRdayof_battery_month{JSON}')
